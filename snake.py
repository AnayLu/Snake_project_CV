...
def choose_mode(cap, handsDetector): #функция выбора сложности в ниндзя и быстрой змейке
    ...
    if results.multi_hand_landmarks is not None:
        ...
        if (x_tip is not None) and (y_tip is not None) and (10<=x_tip<=610) and (750<=y_tip<=900):#условия на проверку куда мы "нажали"
            mode = 1
            crit_time = 6
...
def menu(cap, handsDetector):#функция меню
    ...
    if results.multi_hand_landmarks is not None:
        ...
        if (x_tip is not None) and (y_tip is not None) and(10<=x_tip<=610) and (750<=y_tip<=900):#проверяем на какую кнопку мы "нажали"
            flag = 2
            ...
            flag = 5
    res_image = cv2.cvtColor(flippedRGB, cv2.COLOR_RGB2BGR)#вносим эти прямоугольники на изображение, котрое потом будем показывать
    cv2.putText(res_image, 'Immortal snake', (40, 845), cv2.FONT_HERSHEY_SIMPLEX, 2.2, (0, 0, 0), 6)#печатаем текст на кнопках
...
def end(cap, handsDetector):#функция завершения режимов с отображением результата и возможностью вернуться в меню
    ...
    flippedRGB = cv2.cvtColor(flipped, cv2.COLOR_BGR2RGB)
    results = handsDetector.process(flippedRGB)
    scr, list_capacity = 0, 0#обнуляем доп переменные и списки
    snake = []
    apple_x, apple_y, center = None, None, None
    cv2.rectangle(flippedRGB, (660, 850), (1260, 1000), (154, 214, 143), -1)#рисуем фон для текста и прямоугольник-кнопку для возвращения в меню
    ...
    if status == 'normal':#проверяем если мы не проиграли, то печатаем радостный текст
        cv2.putText(res_image, 'Congratulations!!', (660, 145), cv2.FONT_HERSHEY_SIMPLEX, 2.2, (0, 0, 255), 6)#печатаем текст
    else:#если мы проиграли
        cv2.putText(res_image, 'Game over((', (730, 145), cv2.FONT_HERSHEY_SIMPLEX, 2.2, (0, 0, 255), 6)#печатаем текст
    cv2.putText(res_image, 'Back to menu', (710, 945), cv2.FONT_HERSHEY_SIMPLEX, 2.2, (0, 0, 0), 6)#печатаем текст для кнопки
    if (x_tip is not None) and (y_tip is not None) and (660 <= x_tip <= 1260) and (850 <= y_tip <= 1000):#если нажали на возвращение в меню, то обнуляем остальные переменные
        flag = 0
        score = 0
        mode = 0
...
def immortal_snake(cap, handsDetector, crit_dist, max_score):#режим бессмертной змейки
    ...
    if score == max_score:#если набрали необходмое количество очков, то запускаем окончание
        ...
        cv2.putText(res_image, 'Score: ' + str(score), (60, 120), cv2.FONT_HERSHEY_SIMPLEX, 2.2, (0, 0, 0), 6)#печатаем текст
    ...
    if mode == 0:
        ...
        if score == max_score:#если набрали нужное количество очков завершаем режим
            ...
            if x- start_time>=crit_time:#смотрим сколько времени прошло с момента когда мы последний раз создавали яблоко
                ...
                snake = []
            if apple_x is None or apple_y is None or (x-start_time >= crit_time):#создаем яблоко с произвольными координаты
                apple_x = np.random.randint(30, width - 30)
                apple_y = np.random.randint(30, height - 30)
...
def snake_speedrunner(cap, handsDetector, crit_dist, max_score, crit_time):#режим быстрой змейки
    ...
    if mode == 0:
        ...
        if score == max_score:#если набрали нужное количество очков, то завершаем режим
            ...
            if apple_x is None or apple_y is None or (x - start_time >= crit_time):#если больше чем можно, то делаем новое яблоко
                apple_x = np.random.randint(30, width - 30)
                apple_y = np.random.randint(30, height - 30)
...
def greedy_snake(cap, handsDetector, crit_dist, max_time):#режим жадной змейки
    ...
    if (x-start_time) >= max_time:#если больше чем нужно заканчиваем режим
        scr, list_capacity = 0, 0#обнуляем часть переменных
        snake = []
        apple_x, apple_y, center = None, None, None
        res_image = cv2.cvtColor(flippedRGB, cv2.COLOR_RGB2BGR)
        cv2.rectangle(flippedRGB, (660, 850), (1260, 1000), (154, 214, 143), -1)#рисуем фон для текста и прямоугольник-кноку
        cv2.rectangle(flippedRGB, (610, 50), (1310, 200), (255, 255, 255), -1)
        ...
        if (x_tip is not None) and (y_tip is not None) and (660 <= x_tip <= 1260) and (850 <= y_tip <= 1000):#проверяем нажали ли мы на кнопку возвращения в меню
            flag = 0
            score = 0
            x_tip, y_tip = None, None
...
def mortal_snake(cap, handsDetector, crit_dist, max_score):#режим смертной змейки
    ...
    if score == max_score or status == 'game over':#смотрим набрали ли мы нужное количество очков или умерли
        ...
        if results.multi_hand_landmarks is not None:
            ...
            if (10>=x_tip) or (x_tip+10>=width) or (y_tip<=10) or (y_tip+10>=height):#проверяем если коснулись границ поля, то умираем
                status = 'game over'
            snake2 = []
            ...
            for i in range(len(snake2)):
                for j in range(i+1, len(snake2)):
                    ...
                    if not (p1==p3 or p1==p4 or p2==p3 or p2==p4):#если это два отрезка без общего начала, проверяем на пересечение
